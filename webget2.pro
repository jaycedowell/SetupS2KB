...
PRO MimeType2,  Header, Class, Type, Length
;;
...
FUNCTION webget2,  url,  SILENT=silent, COPYFILE=copyfile, POST=post, $
...
DONE: On_IOERROR, NULL
  ...
  CASE Class OF 
      ...
      'image':BEGIN
          CASE Type OF
              'x-fits': Image = readfits(unit, ImageHeader, Silent=Silent)
	      'fits': Image = readfits(unit, ImageHeader, Silent=Silent)
      ...
      'application':BEGIN 
          CASE Type OF
              'octet-stream':BEGIN ; try reading a FITS file because ESO 
                                   ; answers this way
                  Image = readfits(unit, ImageHeader, Silent=Silent)
               END 
