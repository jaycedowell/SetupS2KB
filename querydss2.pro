PRO QueryDSS2, target, Image,  Header, IMSIZE=ImSIze, ESO=eso, STSCI=stsci, $
              NED=ned, SURVEY = survey, GIF=GIF, OUTFILE = outfile
;+
; NAME: 
;   QueryDSS2
;
...
;
; CALLING SEQUENCE: 
;      QueryDSS2, targetname_or_coords, Im, Hdr, [IMSIZE= , /ESO, /STSCI, /GIF ]
;
; INPUTS:
...
;            otherwise to use the ESO server.    
;
;     /GIF - Download a GIF file instead of a FITS file.  This option will 
;            not return a header.
;
...
;-
  ...
  if N_params() LT 1 then begin
      print,'Syntax - QueryDSS3, TargetName_or_coords, image, header'
      print,"           [Imsize= ,/ESO, /STScI, Survey = ['1','2b','2r','2i'] "
      print,'            /NED, /GIF, OutFile = ]'
      return
   endif
;; Is the user in the Western Hemisphere?
 ...
 endcase
  ...
  IF Keyword_Set(GIF) then form='gif' else form='fits'
  ...
  IF eso THEN $ 
    QueryURL=strcompress("http://archive.eso.org/dss/dss/image?ra="+$
                       ...
                       "&y="+$
                       string(ImSize)+$
                       "&Sky-Survey="+dss +"&mime-type=download-"+$
		       form, /remove) $
  ELSE $
  ...
  if keyword_set(GIF) then OutFile='temp_'+$
	  strcompress(string(10*systime(/Julian),Format='(I)'),/Remove_All)+$
	  '.gif'
